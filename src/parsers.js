...
export default (response) => {
  const channelFeed = response.config.url;
  ...
  const itemsDates = items.map(({ pubDate }) => pubDate);
  const latestItemDate = Math.max(...itemsDates);
...
export const parseUpdate = ({ data }, channelId) => {
  const parser = new DOMParser();
